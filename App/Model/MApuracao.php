...
class MApuracao {
	...
	public function apuracaoExcluida($post, $idApuracao, $idUsuario)
	{
		...
		$sql->query("
			INSERT INTO tb_apuracaoexcluida (idCriarApuracao, idUsuario, motivo, status) 
			VALUES(:idCriarApuracao, :idUsuario, :motivo, :status)
		", [
			":idCriarApuracao" => $idApuracao,
			":idUsuario" => $idUsuario,
	...
	public function listApuracaoCompletaExcluida()
	{
		$sql = new Conexao;

		return $sql->select("
			SELECT 
			a.idApuracaoExcluida, a.idUsuario idUsuarioExcluiu, a.motivo motivoExclusao, a.dataRegistro dataRegistroExclusao, 
			b.idCriarApuracao, b.idUsuario, b.tipoApuracao, b.descricao, b.status, b.dataCriacao, b.dataRegistro,
			c.idVitimasCriarApuracao, c.idVitimasApuracao,
			d.idPessoa,
			e.qualFamilia,
			...
			INNER JOIN tb_endereco l ON h.idEndereco = l.idEndereco
			INNER JOIN tb_endereco m ON i.idEndereco = m.idEndereco
			WHERE b.status = 4 AND a.status = 0
			ORDER BY a.idCriarApuracao DESC
		");
	}

	public function listApuracaoExcluida($idApuracao)
	{
		$sql = new Conexao;
		...
		return $sql->select("
			...
			INNER JOIN tb_pessoa o ON n.idPessoa = o.idPessoa
			INNER JOIN tb_usuario p ON a.idUsuario = p.idUsuario
			INNER JOIN tb_pessoa q ON p.idPessoa = q.idPessoa
			WHERE a.idCriarApuracao = :idCriarApuracao AND b.status = 4 AND a.status = 0;
		", [
			":idCriarApuracao" => $idApuracao
		]);
	}

	public function editarStatusApuracaoExcluida($idApuracaoExcluida)
	{
		$sql = new Conexao;

		$sql->query("
			UPDATE tb_apuracaoexcluida 
			SET status = :status
			WHERE idApuracaoExcluida = :idApuracaoExcluida
		", [
			":status" => 1,
